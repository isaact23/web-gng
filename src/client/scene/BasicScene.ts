  ...
  loadChunk(chunk: IChunk) : void {
    ...
    for (let [coordinate, block] of blockIterator) {
      ...
      if (aboveBlock == Block.Air || aboveBlock == undefined) {
        ...
        top.position = new Babylon.Vector3(coordinate.x, coordinate.y + 0.5, coordinate.z);
        top.rotation = new Babylon.Vector3(Math.PI / 2, 0, 0);
        top.freezeWorldMatrix();
      }

      const belowBlock = chunk.getBlock(coordinate.x, coordinate.y - 1, coordinate.z);
      if (belowBlock == Block.Air || belowBlock == undefined) {
        ...
        bottom.position = new Babylon.Vector3(coordinate.x, coordinate.y - 0.5, coordinate.z);
        bottom.rotation = new Babylon.Vector3(3 * Math.PI / 2, 0, 0);
        bottom.freezeWorldMatrix();
      }

      const frontBlock = chunk.getBlock(coordinate.x + 1, coordinate.y, coordinate.z);
      if (frontBlock == Block.Air || frontBlock == undefined) {
        ...
        front.position = new Babylon.Vector3(coordinate.x + 0.5, coordinate.y, coordinate.z);
        front.rotation = new Babylon.Vector3(0, 3 * Math.PI / 2, 0);
        front.freezeWorldMatrix();
      }

      const backBlock = chunk.getBlock(coordinate.x - 1, coordinate.y, coordinate.z);
      if (backBlock == Block.Air || backBlock == undefined) {
        const back = Babylon.MeshBuilder.CreatePlane("back", {width: 1, height: 1}, this.scene);
        back.position = new Babylon.Vector3(coordinate.x - 0.5, coordinate.y, coordinate.z);
        back.rotation = new Babylon.Vector3(0, Math.PI / 2, 0);
        back.freezeWorldMatrix();
      }

      const leftBlock = chunk.getBlock(coordinate.x, coordinate.y, coordinate.z - 1);
      if (leftBlock == Block.Air || leftBlock == undefined) {
        const left = Babylon.MeshBuilder.CreatePlane("left", {width: 1, height: 1}, this.scene);
        left.position = new Babylon.Vector3(coordinate.x, coordinate.y, coordinate.z - 0.5);
        left.rotation = new Babylon.Vector3(0, 0, 0);
        left.freezeWorldMatrix();
      }

      const rightBlock = chunk.getBlock(coordinate.x, coordinate.y, coordinate.z + 1);
      if (rightBlock == Block.Air || rightBlock == undefined) {
        ...
        right.position = new Babylon.Vector3(coordinate.x, coordinate.y, coordinate.z + 0.5);
