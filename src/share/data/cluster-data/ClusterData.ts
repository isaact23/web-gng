...
export class ClusterData implements IClusterData {
  ...
  private constructor(private chunks: SerializableGrid<IChunkData, IChunkCoordinate>) {}

  /**
   * Create an empty ClusterData.
   * @returns New empty ClusterData object.
   */
  static new(): ClusterData {
    let grid = SerializableGrid.new<IChunkData, IChunkCoordinate>();
    return new ClusterData(grid);
  }

  /**
   * Create a ClusterData object from a string representation.
   * @param rep The string representation generated by toStringRep().
   * @returns ClusterData populated with chunks from rep.
   */
  static fromStringRep(rep: string): ClusterData {
    let chunks = SerializableGrid.fromStringRep<IChunkData, IChunkCoordinate>(
      rep, str => ChunkData.fromStringRep(str));
   ...
   */
  toStringRep(): string {
    return this.chunks.toStringRep(chunk => chunk.toStringRep());
  }
